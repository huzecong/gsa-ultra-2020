...
def inner_prod_3(a: List[T], b: List[T]) -> T:
    return a[0] * b[0] + a[1] * b[1] + a[2] * b[2]


def piecewise_prod_3(a: List[T], b: List[T]) -> List[T]:
    return [a[0] * b[0], a[1] * b[1], a[2] * b[2]]
    ...
    def top_down_dfs(x: int, top_down_prob: List[Frac], parent: Optional[int] = None) -> None:
        ...
        for child in edges[x]:
            ...
            child_prob = [bottom_up_probs_excluded[child][i] * inner_prod_3(trans_probs[i], top_down_prob)
                          for i in range(3)]
            child_prob2 = [node_prob[i] / sum(trans_probs[i][j] * bottom_up_probs[child][j] for j in range(3))
                           for i in range(3)]
...
def main():
    ...
    parent = [-1] + [random.randint(0, idx) for idx in range(n)]
